    ...
    def init_weights(self):
        ...
        for i in range(self.n_inputs):
            ...
            for j in range(self.n_hidden_neurons):
                self.v0.append(float(randint(-10,10)) * beta/10.0)
                self.v[i].append(float(randint(-50,50))/100.0)


        # Hidden neurons (2nd layer)
        for j in range(self.n_hidden_neurons):
            self.vv.append([])

            # 1st layer -> 2nd layer weights and bias
            for l in range(self.n_hidden_neurons):
                self.vv0.append(float(randint(-10,10)) * beta/10.0)
                self.vv[j].append(float(randint(-50,50))/100.0)

        # Nguyen-Widrow Initialization (1st hidden layer)
        for j in range(self.n_hidden_neurons):
            norma_vj = 0
            for i in range(self.n_inputs):
        ...
        # Nguyen-Widrow Initialization (2nd hidden layer)
        for l in range(self.n_hidden_neurons):
            norma_vvj = 0
            for j in range(self.n_hidden_neurons):
                norma_vvj += pow(self.vv[j][l], 2)

            norma_vvj = sqrt(norma_vvj)

            for j in range(self.n_hidden_neurons):
                self.vv[j][l] = (beta * self.vv[j][l]) /norma_vvj
...
# ------------------------------------------------------------------------------
    ...
    def calc_2nd_net(self):
        ...
        for l in range(self.n_hidden_neurons):
            ...
            for j in range(self.n_hidden_neurons):
                try:
                    self.net += self.z[j] * self.vv[j][l]
                except(IndexError):
                    print ('IndexError exception caught!')
                    print ('z, vv = ', self.z, self.vv)
                    exit(-1)
                self.net += self.vv0[j]

            self.zz_in.append(self.net)
            
        self.activation_2nd_function()
...
# ------------------------------------------------------------------------------
    ...
    # Activation function
    def activation_2nd_function(self):
        self.zz = []
        for j in range(self.n_hidden_neurons):
            try:
                if self.type == 'bin':
                    self.zz.append(1.0/(1.0 + exp(-self.zz_in[j])))
                else:
                    self.zz.append(2.0/(1.0 + exp(-self.zz_in[j])) - 1.0)
            except OverflowError:
                print ('OverflowError exception reached. Execution aborted.')
                exit(-1)
        ...
        for j in range(self.n_hidden_neurons):
            ...
            for k in range(self.n_output_neurons):
                if (self.delta_wjk_old.__len__() == 0):
                    self.delta_wjk[j].append(self.alpha * self.delta_k[k] * self.zz[j])
                else:
                    self.delta_wjk[j].append(self.alpha * self.delta_k[k] * self.zz[j] + self.mu * self.delta_wjk_old[j][k])
...
# ------------------------------------------------------------------------------
    ...
    # Derivative
    def bp_2nd_hidden(self):

        self.delta_vjl_old = deepcopy(self.delta_vij)
        ...
        for l in range(self.n_hidden_neurons):
            self.delta_in_l.append(0)

            # Calculate delta_in_j
            for k in range(self.n_output_neurons):
                self.delta_in_l[l] += self.delta_k[k] * self.w[l][k]

            if self.type == 'bin':
                derivative = self.zz[l] * (1 - self.zz[l])
            else:
                derivative = 0.5 * (1.0 + self.zz[l]) * (1.0 - self.zz[l])

            # Calculate delta_j
            self.delta_l.append(self.delta_in_l[l] * derivative)
        
        # Calculate delta_vjl
        for j in range(self.n_hidden_neurons):
            self.delta_vjl.append([])        
            for l in range(self.n_hidden_neurons):
                if (self.delta_vjl_old.__len__() == 0):
                    self.delta_vjl[j].append(self.alpha * self.delta_l[l] * self.z[j])
                else:
                    self.delta_vjl[j].append(self.alpha * self.delta_l[l] * self.z[j] + self.mu * self.delta_vjl_old[j][l])
                
        # Calculate delta_v0j
        for l in range(self.n_hidden_neurons):
            self.delta_v0l.append(self.alpha * self.delta_l[l])
...
# ------------------------------------------------------------------------------
    ...
    def bp_hidden(self, input):
        ...
        for j in range(self.n_hidden_neurons):
            ...
            # Calculate delta_in_j
            for l in range(self.n_hidden_neurons):
                self.delta_in_j[j] += self.delta_l[l] * self.vv[j][l]
...
# ------------------------------------------------------------------------------
    ...
    # Update weights from output neurons
    def hidden_2nd_update(self):
        for l in range(self.n_hidden_neurons):
            self.vv0[l] += self.delta_v0l[l]
            for j in range(self.n_hidden_neurons):
                self.vv[j][l] += self.delta_vjl[j][l]
...
if __name__ == "__main__":
    ...
    for line in lines:
        patterns = line.split()
        training_set.append( ([float(patterns[0]), float(patterns[1]), float(patterns[2]), float(patterns[3]), float(patterns[4])], [int(patterns[5]), int(patterns[6]), int(patterns[7]), int(patterns[8])]))

    for n in range(2,6,1):

        n_hidden_neurons = n

        for tolerance in range(150,301,50):
            print ('-----------')
            print ('Test header')
            print ('-----------')
            ...
            for test in range(500):
                ...
                a1 = BP()
                a1.update_parameters(training_set, alpha, tolerance, n_hidden_neurons, n_output_neurons, 'bip')
                ...
                for line in lines:
                    patterns = line.split()
                    test_set.append([float(patterns[0]), float(patterns[1]), float(patterns[2]), float(patterns[3]), float(patterns[4])])
                    result_set.append([int(patterns[5]), int(patterns[6]), int(patterns[7]), int(patterns[8]), int(patterns[9])])

                correct = 0
                total = 0

                correct_vector = [0 for i in range(20)]
                total_vector =[0 for i in range(20)]
                #print 'correct_vector',correct_vector
                #print 'total_vector--',total_vector
                #print 'test_set.__len__()',test_set.__len__()
                ...
                for i in range(test_set.__len__()):
                    ...
                    #print 'result',result
                    #print 'result_set[i]',result_set[i]
                    for j in range(n_output_neurons):
                ...
                fid = open('results/res_'+str(n_hidden_neurons)+'_'+str(tolerance)+'.dat','a')
                fid.write(str(tolerance) + '\t' + str(float(window_correct)/correct_vector.__len__()) + '\t' + str(delta_t) + '\n')
                fid.close()
