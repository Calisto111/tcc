...
# ------------------------------------------------------------------------------
    ...
    def calc_all_Ar(self):
        for i in range(self.n_segments):
            if i == 0:
                print ("Segmento(",i,") de ",self.n_segments, "Arquivo yhat:", self.yhatfile,"Vetor de AR inicial: ",self.AR_vect, "Ordem do Filtro", self.filterOrder, "Yhat inicial", self.yhat, "Constante de Convergencia", self.cteCng)
                self.segments[i].calc_AR(i,self.yhatfile,self.n_segments,self.AR_vect,self.filterOrder,self.yhat,self.cteCng)
            else:
                self.segments[i].calc_AR(i,None,self.n_segments,None,self.filterOrder,None,self.cteCng)
...
# ------------------------------------------------------------------------------
    ...
    def calc_AR(self,rightsegment,yhatfile,n_segments,AR_vect,filterOrder,yhat,cteCng):
        ...
        global RECURSAO_AR_VECT
        global BUFFER_VECT
        global FILEYHAT
        global CONT

        if self.rightsegment != 0:
            #Recursao
            #print "Recursao", RECURSAO_AR_VECT
            self.AR_vect = copy(RECURSAO_AR_VECT)
            self.BufferTemp = copy(BUFFER_VECT)
            self.rightsegment = rightsegment
            self.n_segments = n_segments
            self.yhat = []
            self.yhatfile = ''
            print ("Segmento(",self.rightsegment,") de ",self.n_segments, "Ordem do Filtro", self.filterOrder,"Recursao AR_vect:",RECURSAO_AR_VECT,"Recursao BufferVect:",BUFFER_VECT,"Vetor de AR: ",self.AR_vect,"Quantos ARs:",len(self.AR_vect))
#"Yhat", self.yhat,"Quantos yhat",len(self.yhat), "Constante de Convergencia", self.cteCng,"Arquivo yhat:", self.yhatfile,
        else:
            self.AR_vect = copy(AR_vect)
            self.yhat = yhat
            self.BufferTemp=[]
            self.yhatfile = yhatfile
        ...
        for i in range (self.length):
            ...
            if self.rightsegment == 0 and self.rightsegment < self.n_segments:
                #Saving last N samples
                if i == (self.length- (self.filterOrder+1)):
                    self.BufferTemp=[]
                    #Updating Buffer
                    for b in range ((i+1),self.length):
                        self.BufferTemp.append(self.data_vector[b])
                    self.BufferTemp.reverse()
                    BUFFER_VECT = copy(self.BufferTemp)
                for j in range(self.filterOrder):
                    #print "I, J:", i, j
                    if i == 0:
                        somaYhat+= 0.0
                ...
                #Saving last N samples
                #print 'I',i,"diferrenca do si",(self.length-(self.filterOrder+1))
                if i == (self.length- (self.filterOrder+1)):
                    self.BufferTemp=[]
                    #Updating Buffer
                    for b in range ((i+1),self.length):
                ...
                for j in range(self.filterOrder):
                    #print "I, J:", i, j
                    if i == 0:
                        somaYhat+=self.AR_vect[i][j]*(float(self.BufferTemp[j])/10000)
                    if i == 1 :
                        if j >= (self.filterOrder-i):
                            somaYhat+=self.AR_vect[i][j]*(float(self.data_vector[j-i])/10000)
                        else:
                            somaYhat+=self.AR_vect[i][j]*(float(self.BufferTemp[j])/10000)                            
                    if i == 2 :
                        if j >= (self.filterOrder-i):
            ...
            if self.rightsegment == 0 and self.rightsegment < self.n_segments:
                ...
                if (self.length-1) != i:
                    ...
                    #Recurssao
                    RECURSAO_AR_VECT=copy([self.temp_AR])                        
            else:
                for j in range(self.filterOrder):
                    #print "I, J:", i, j
                    if i == 0:
                        self.temp_AR[j]=self.AR_vect[i][j]-2*self.cteCng*erro*(float(self.BufferTemp[j])/10000)
                    if i == 1 :
                        if j >= (self.filterOrder-i):
                            self.temp_AR[j]=self.AR_vect[i][j]-2*self.cteCng*erro*(float(self.data_vector[j-i])/10000)
                        else:
                            self.temp_AR[j]=self.AR_vect[i][j]-2*self.cteCng*erro*(float(self.BufferTemp[j])/10000)
                    if i == 2 :
                        if j >= (self.filterOrder-i):
                    ...
                    if i == 8 :
                        if j >= (self.filterOrder-i):
                            ...
                            self.temp_AR[j]=self.AR_vect[i][j]-2*self.cteCng*erro*(float(self.BufferTemp[j])/10000) 
                    if i >= 9:
                        self.temp_AR[j]=self.AR_vect[i][j]-2*self.cteCng*erro*(float(self.data_vector[i-j])/10000)
                if (self.length-1) != i:
                    self.AR_vect.append(self.temp_AR)
                else:
                    #Recurssao
                    RECURSAO_AR_VECT=copy([self.temp_AR])
        self.segment_ARs = copy(self.AR_vect)
        print ("Segment_ARs",self.segment_ARs)
        print ("Quantidade de Segment_ARs",len(self.segment_ARs))
        print ("self.rightsegment",self.rightsegment)
        print ("self.n_segments",self.n_segments-1)
        if self.rightsegment == self.n_segments - 1:
            path = 'yhat0'+str(CONT)+'.dat'
            print ("path",path)
            fid = open(path,'w')
            fid.writelines(FILEYHAT)
            fid.close
            CONT+=1
        else:
            print ("Escrevendo no arquivo")
            FILEYHAT= FILEYHAT+self.yhatfile
